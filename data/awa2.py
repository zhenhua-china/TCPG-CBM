...
class AnimalDataset(Dataset):
  def __init__(self, data_path, transform):
    root_dir = os.path.dirname(os.path.abspath(data_path))
    predicate_binary_mat = np.array(np.genfromtxt(os.path.join(root_dir, 'predicate-matrix-binary.txt'), dtype='int'))
    ...
    img_names = [os.path.normpath(p) for p in df['img_name'].tolist()]
  ...
  def __getitem__(self, index):
    ...
    return {
      'image': im,
      'class_label': torch.tensor(im_index, dtype=torch.long),
      'concept_labels': torch.FloatTensor(im_predicate)
    }
...
def generate_data(config, root_dic=AWA2_DATA_DIR, seed=42, output_dataset_vars=False, resol=224, is_training=True):
    ...
    train_dl = load_data(os.path.join(root_dic, 'train.csv'), config.batch_size, shuffle=True, num_workers=0, resol=resol, is_training=True)
    
    val_dl = load_data(os.path.join(root_dic, 'val.csv'), config.batch_size, shuffle=False, num_workers=0, resol=resol, is_training=False)
    
    test_dl = load_data(os.path.join(root_dic, 'test.csv'), config.batch_size, shuffle=False, num_workers=0, resol=resol, is_training=False)
